...
Moreover, akin to `pplr_report_dictionary`, browse can generate a report and open it as an html page. To do so, set the `report` variable to TRUE:   
...
1. By providing `pplr_get_data` with an object created subsetting the metadata table using `pplr_browse()` 
1. By further subsetting an object created through `pplr_browse()`
1. By subsetting the popler database directly through `pplr_get_data()`

A few examples on how to the use of `pplr_get_data()` are:

``` {r eval = FALSE}
poa_metadata <- pplr_browse(genus == "Poa" & species == "fendleriana")
poa_d        <- pplr_get_data(poa_metadata)
poa_d_09     <- pplr_get_data(poa_metadata, year > 2008)
parasite_d   <- pplr_get_data(proj_metadata_key == 25)
...
Using the `pplr_cov_unpack` function on a data frame downloaded using `pplr_get_data` returns a data frame of the covariates contained in the downloaded object.
