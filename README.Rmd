...
3. This version is not passing `goodpractice::gp()` required by _ropensci_. Currently, the function itself fails on the package due to a malformed file error. I've traced this back to `R/report_metadata.R` and found that others have had [issues](https://github.com/jimhester/lintr/issues/252) with running **lintr**, so maybe we are having similar problem. I have not yet had time to scan through the function for the actual source of the error yet though.
