...
# Create a summary table containing names of replication levels that contain 3 samples per year for 15 years from the Sevilleta LTER. 
SEV_long_studies <- pplr_site_rep(SEV, freq = 3, duration = 15, return_logical = FALSE)


# you can also just subset it directly using the function and specifying it to return a logical vector
...
`popler` contains a wealth of taxonomic information from each project. To select information based on taxonomic groups, simply specify which group and which category you wish to select. The default settings of popler provide seven taxonomic groups: kingdom, phylum, class, order, family, genus, and species in each request. Additionally, a column called `sppcode` is included.
...
Popler allows carrying out more complicated searches than those suggested above. In particular, it allows to i) combine to simultaneously search several types of metadata variables, and ii) use keywords. In the first case, simply provide the function `pplr_browse()` with a logical statement regarding more than one metadata variable. For example, if you want studies on plants with at least 4 nested spatial levels, and 10 years of data:
